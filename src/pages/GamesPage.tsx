  ...
  return (
    <div className="flex flex-col h-full px-5 py-6 overflow-y-auto">
      ...
      <TipReveal
        ...
        summary={`Level ${level}, ${xp}/${xpNeeded} XP. ${xpNeeded - xp} XP to Lv ${level + 1}.`}
        tips={[
      ...
      >
        <div className="glass-card rounded-3xl p-5 health-glow-progress">
      ...
      <TipReveal
        colorClass="text-health-progress"
        summary={`Daily ${metrics.healthScore} · Monthly ${Math.round(metrics.healthScore * 0.95)} · Yearly ${Math.round(metrics.healthScore * 0.91)}.`}
        tips={[
      ...
      <TipReveal
        ...
        summary={stepsToFirst > 0
          ? `You're #${yourRank} of ${leaderboard.length}. ${stepsToFirst.toLocaleString()} steps from #1.`
          : `🏆 You're in 1st place!`}
        tips={
          stepsToFirst > 0
            ? [
                ...
                "Walking meetings or treadmill desks rack up steps fast.",
      ...
      <TipReveal
        colorClass="text-health-progress"
        summary={`${achievements.filter(a => a.unlocked).length} of ${achievements.length} unlocked.`}
        tips={[
          "Daily consistency beats heroic single-day efforts.",
          "Share milestones with friends for bonus XP.",
          "Challenge yourself with a higher tier after each unlock.",
          "Locked achievements show your % progress — focus on those near 80%+.",
        ]}
      >
      ...
      <TipReveal
        ...
        summary={`${challenges.filter(c => c.joined).length} active challenges. Complete them for bonus XP.`}
        tips={[
          "Pick 1-2 challenges max — focus beats spreading thin.",
          "Plan a hike or long walk on weekends for step challenges.",
          "Set clothes out the night before for early-morning challenges.",
          "Keep a water bottle visible for hydration challenges.",
        ]}
      >
